...
function BorrowedList() {
	...
	return (
		<div id="book-list">
			<div id="search">
     			 <input type="text" name="search" placeholder="Hover here to look for a book..." id="search-bar" onChange={search_function}></input>
      		</div>
			<div id="list-title">
			...
			<li>
				{books.length > 0 &&
					books.filter(
							({ title, author, description, situation }) =>
								(title.toLowerCase().includes(search) ||
								author.toLowerCase().includes(search) ||
								description.toLowerCase().includes(search)) && situation=='Borrowed from').map(
